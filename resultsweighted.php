...
$difmin = $_POST['difmin'];
$difmax = $_POST['difmax']
$difmid = array range($difmin+1,$difmax-1);
$count = $_POST['songcount'];
$leg = $_POST['legg'];
$wmin = $_POST['weightmin'];
$wmid = $_POST['weightmid'];
$wmax = $_POST['weightmix'];

if ($wmin+$wmid+$wmax!='100') {
  die("Weights are incorrect. Please make sure they are set to total 100);
}
...
if (isset($_POST['legg'])) {
        ...
        $sql = "DECLARE @Percent INT, @Random float
                
                SELECT @Percent = '$wmax' * RAND()
                
                SELECT genre,song,artist,spn,sph,spa,songid,style 
                FROM Songs 
